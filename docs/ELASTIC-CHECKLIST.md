...
9. Run `make workflow-live` to validate and save the workflow disabled. `make workflow-agent-test` proves the constrained evidence-gap step; the observed run completed in 34,375 ms and used 21,584 tokens. `make workflow-human-gate-test` proves that a matching native `waitForInput` gate pauses and resumes with `hold`. These are component tests: enable or attach the alert-triggered workflow only after one complete controlled end-to-end run succeeds.
