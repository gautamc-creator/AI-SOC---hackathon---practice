...
2. **Detection (30 sec):** Show the typed VIGIL Kibana dashboard: 12 correlated events and the login → egress → payment timeline. State that the data is synthetic, then briefly show the three behavior-based alerts.
...
5. **Human accountability (30 sec):** Show that Elastic's native gate can pause for `approve`, `hold`, or `reject`, then show the same `hold` decision in the War Room. Emphasize that VIGIL does not automatically contain systems or submit reports.
