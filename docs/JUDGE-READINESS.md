...
The prototype is competitive but not presentation-complete. The native Kibana evidence dashboard, controlled Agent Builder step, and resumable human gate are now proven. The remaining high-value work is one full alert-triggered workflow rehearsal, sponsor-model integration at the event, capture of permitted backup evidence, and repeated stage rehearsal. Do not add a second scenario until this path is visually flawless.
...
| Elastic Workflow | Human-gated workflow validates and is saved disabled. Controlled `ai.agent` execution completed in 34,375 ms; a separate native `waitForInput` harness paused, accepted `hold`, and resumed. | Component execution is proven; the full alert-triggered chain is not yet proven end to end. |
...
| Technical / Elastic | Data stream, ingest pipeline, threshold + EQL rules, Attack Discovery, ES|QL lookup, typed dashboard, Security Case, Agent Builder and native human gate. | Strong live backend and UI proof. | One full alert-triggered Elastic Workflow run. |
...
1. Run `elastic/workflows/vigil-human-gated-triage.yaml` end to end: Attack Discovery trigger → create case → attach alerts → Agent Builder evidence-gap review → native human decision → decision comment. The workflow is valid and saved disabled; full-chain execution must remain labelled unproven until one run completes. Do **not** auto-isolate a synthetic host merely for spectacle.
