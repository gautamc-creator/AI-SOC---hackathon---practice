...
| `event.category` | `authentication`, `network`, `transaction` | Separates the incident stages. |
...
| `transaction.amount`, `transaction.currency`, `transaction.channel` | `175000`, `INR`, `UPI` | Synthetic fixture impact detail. |
...
This index uses Elasticsearch `index.mode: lookup`. It contains only two synthetic service records. `host.name` is the join key. `bank.exposure_inr` and `bank.account_count` are fixed fixture context, not a real loss model.
